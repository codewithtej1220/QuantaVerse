 ...
 * A lesson's video.
 *
 * `url` is whatever you paste: a YouTube or Vimeo watch link, or a direct file.
 * The player works out which it is rather than making the author declare it,
 * because the author is going to paste an address off the clipboard and a field
 * that also demands a provider name is a field that will be filled in wrong.
 *
 * Absent is a real state, not a broken one. A lesson with no recording yet
 * shows a slot that says so and the theory below it reads normally — far better
 * than an embed pointed at a URL that does not exist, which renders as a dead
 * grey rectangle and takes the lesson's credibility with it.
 */
export interface LessonVideo {
  url: string;
  /** Runtime, shown before it is played. */
  minutes?: number;
  /** One line on what the recording covers, for anyone who cannot watch it. */
  caption?: string;
}

/** One question on the end-of-module test. */
 ...
 *
 * It opens once the lab is passed, and that order is the point. The lab asks
 * whether you can build the thing; the test asks whether you know why it
 * worked. Putting the test first would let somebody pass it from the prose
 * without ever touching a circuit, which is the failure mode this whole site
 * is arguing against.
 */
export const MODULE_TESTS: Record<string, TestQuestion[]> = {
  "qubit-and-superposition": [
    {
      prompt: "What does it take to write down the state of a single qubit?",
      options: [
        "One bit: it is either 0 or 1",
        "Two complex amplitudes, with squared magnitudes summing to one",
        "Two probabilities summing to one",
        "One real angle",
      ],
      answer: 1,
      because:
        "Two complex amplitudes. Two probabilities would not be enough — they throw away the phase, which is exactly the part that lets amplitudes cancel.",
    },
    {
      prompt: "|+⟩ and |−⟩ give identical measurement statistics in the computational basis. What is different about them?",
      options: [
        "Nothing — they are the same state written two ways",
        "Their global phase",
        "The relative phase between the |0⟩ and |1⟩ amplitudes",
        "Their normalisation",
      ],
      answer: 2,
      because:
        "The relative phase: one has a minus sign on the |1⟩ amplitude. It is invisible to a measurement in this basis and entirely visible after a Hadamard.",
    },
    {
      prompt: "You apply H to |0⟩, then H again, then measure. What do you see?",
      options: [
        "50/50 between 0 and 1",
        "0 with certainty",
        "1 with certainty",
        "25/75 between 0 and 1",
      ],
      answer: 1,
      because:
        "0, every time. H is its own inverse, so the second one rotates the state exactly back to where it started.",
    },
    {
      prompt: "Why can a fair coin not reproduce that two-Hadamard result?",
      options: [
        "A coin is biased in practice",
        "A coin has only two outcomes, and a qubit has more",
        "The first H left a definite state that could be rotated back, whereas a landed coin is a result that cannot be un-flipped",
        "It can — the experiment does not distinguish them",
      ],
      answer: 2,
      because:
        "This is the heart of the module. Superposition is a definite state, not a hidden outcome. Rotations are reversible; randomness is not.",
    },
    {
      prompt: "Multiplying an entire qubit state by e^(iγ) changes which of these?",
      options: [
      ...
      answer: 2,
      because:
        "Nothing observable — that is what makes it global phase. It is why the Bloch sphere needs two numbers rather than three, and why the graded labs compare up to it.",
    },
    {
      prompt: "On the Bloch sphere, what sits on the equator?",
      options: [
        "States that are certain to measure 0",
        "States that are certain to measure 1",
        "States with an equal chance of 0 and 1, differing by phase around the circle",
        "Entangled states",
      ],
      answer: 2,
      because:
        "Equal superpositions. Going around the equator changes the phase, not the probabilities — which is why |+⟩ and |−⟩ both live there, opposite each other.",
    },
  ],
};

export function testFor(slug: string): TestQuestion[] {
  return MODULE_TESTS[slug] ?? [];
}
